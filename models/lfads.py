...
class LFADS_Net(nn.Module):
    '''
    LFADS_Net (Latent Factor Analysis via Dynamical Systems) neural network 
    class.
    
    __init__(self, input_size, factor_size=4, g_encoder_size=64, 
             c_encoder_size=64, g_latent_size=64, u_latent_size=1,
             controller_size=64, generator_size=64, prior=PRIOR, 
    ...
    def __init__(self, input_size, factor_size=4, g_encoder_size=64, 
                 c_encoder_size=64, g_latent_size=64, u_latent_size=1,
                 controller_size=64, generator_size=64, prior=PRIOR, 
                 clip_val=5.0, dropout=0.0, max_norm=200, deep_freeze=False, 
                 do_normalize_factors=True, factor_bias=False, device='cpu'):
...
#         self.output_size          = input_size if output_size is None else output_size
        ...
        if (self.c_encoder_size > 0 and self.controller_size > 0 and 
            self.u_latent_size > 0):
            self.controller  = LFADS_ControllerCell(
                input_size      = self.c_encoder_size*2 + self.factor_size,
        ...
        if self.g_latent_size == self.generator_size:
            self.fc_genstate = Identity(
                in_features=self.g_latent_size, 
                out_features=self.generator_size
                )
        else:
            self.fc_genstate = nn.Linear(
                in_features= self.g_latent_size, 
                out_features= self.generator_size
                )
        ...
        if (self.c_encoder_size > 0 and self.controller_size > 0 and 
            self.u_latent_size > 0):
            self.c_encoder_init  = nn.Parameter(
                torch.zeros(2, self.c_encoder_size)
                )
            self.controller_init = nn.Parameter(
                torch.zeros(self.controller_size)
                )
...
#         self.register_buffer('g_posterior_logvar',None)
    ...
    def forward(self, input):
        ...
        Required Arguments:
            - input (torch.Tensor): input data with dimensions 
                                    [time x batch x cells]
        '''

        # Initialize hidden states
        [g_encoder_state, 
         c_encoder_state, 
         controller_state] = self.initialize_hidden_states(input) 

        
        # Encode input and calculate and calculate generator initial condition 
        # variational posterior distribution
        [self.g_posterior_mean, 
         self.g_posterior_logvar, 
         out_gru_g_enc, 
         out_gru_c_enc] = self.encoder(
             input, (g_encoder_state, c_encoder_state)
             )
        

        # Sample generator state
        generator_state = self.fc_genstate(
            self.sample_gaussian(self.g_posterior_mean, self.g_posterior_logvar)
            )
        ...
        if (self.c_encoder_size > 0 and self.controller_size > 0 and 
            ...
            gen_inputs = torch.empty(
                0, self.batch_size, self.u_latent_size, device=self.device
                )
            
            # initialize u posterior store
            self.u_posterior_mean   = torch.empty(
                self.batch_size, 0, self.u_latent_size, device=self.device
                )
            self.u_posterior_logvar = torch.empty(
                self.batch_size, 0, self.u_latent_size, device=self.device
                )
        ...
        for t in range(self.steps_size):
            if (self.c_encoder_size > 0 and self.controller_size > 0 and 
                self.u_latent_size > 0):
                # Update controller state and calculate generator input 
                # variational posterior distribution
                u_mean, u_logvar, controller_state = self.controller(
                    torch.cat((out_gru_c_enc[t], factor_state), dim=1), 
                    controller_state
                    )
                
                # Append u_posterior mean and logvar
                self.u_posterior_mean = torch.cat(
                    (self.u_posterior_mean, u_mean.unsqueeze(1)), dim=1
                    )
                self.u_posterior_logvar = torch.cat(
                    (self.u_posterior_logvar, u_logvar.unsqueeze(1)), dim=1
                    )
                ...
                gen_inputs  = torch.cat(
                    (gen_inputs, generator_input.unsqueeze(0)), dim=0
                    )
            else:
                generator_input = torch.empty(
                    self.batch_size, self.u_latent_size, device=self.device
                    )
                gen_inputs = None
                
            # Update generator and factor state
            generator_state, factor_state = self.generator(
                generator_input, generator_state
                )
            # Store factor state
        ...
        if (self.c_encoder_size > 0 and self.controller_size > 0 and 
            ...
            self.u_prior_mean, self.u_prior_logvar = self._gp_to_normal(
                self.u_prior_gp_mean, 
                self.u_prior_gp_logvar, 
                self.u_prior_gp_logtau, 
                gen_inputs
                )
    ...
    def initialize_hidden_states(self, input):
        ...
        g_encoder_state  = (torch.ones(
            self.batch_size, 2, self.g_encoder_size, device=self.device
            ) * self.g_encoder_init).permute(1, 0, 2)

        if (self.c_encoder_size > 0 and self.controller_size > 0 and 
            self.u_latent_size > 0):
            c_encoder_state  = (torch.ones(
                self.batch_size, 2, self.c_encoder_size, device=self.device
                ) * self.c_encoder_init).permute(1, 0, 2)
    ...
    def _gp_to_normal(self, gp_mean, gp_logvar, gp_logtau, process):
        ...
        mean   = gp_mean * torch.ones(
            1, process.shape[1], process.shape[2], device=self.device
            )
        logvar = gp_logvar * torch.ones(
            1, process.shape[1], process.shape[2], device=self.device
            )
        
        process_mean = \
            gp_mean + (process[:-1] - gp_mean) * torch.exp(-1 / gp_logtau.exp())
        process_logvar = (
            torch.log(1 - torch.exp(-1 / gp_logtau.exp()).pow(2)) + 
            gp_logvar * torch.ones(
                process.shape[0] - 1, process.shape[1], process.shape[2], 
                device=self.device
                )
            )
        mean   = torch.cat((mean, process_mean))
        logvar = torch.cat((logvar, process_logvar))
    ...
    def normalize_factors(self):
        self.generator.fc_factors.weight.data = F.normalize(
            self.generator.fc_factors.weight.data, 
            dim=1
            )
...
class LFADS_SingleSession_Net(LFADS_Net):
    ...
    def __init__(self, input_size, factor_size=4, g_encoder_size=64, 
        ...
        super(LFADS_SingleSession_Net, self).__init__(
        ...
        if self.obs == 'poisson':
            self.fc_logrates = nn.Linear(
                in_features=self.factor_size, out_features=self.input_size
                )
        elif self.obs == 'gaussian':
            self.fc_out = nn.Linear(
                in_features=self.factor_size, out_features=self.input_size
                )
            self.logvar = nn.Parameter(torch.zeros(self.input_size))
        
        self.initialize_weights()
        
    def forward(self, input):
        factors, gen_inputs = super(LFADS_SingleSession_Net, self).forward(
            input.permute(1, 0, 2)
            )
...
class LFADS_MultiSession_Net(LFADS_Net):
    ...
    def __init__(self, W_in_list, W_out_list, b_in_list, b_out_list, 
                 factor_size=4, g_encoder_size=64, c_encoder_size=64,
                 g_latent_size=64, u_latent_size=1, controller_size=64, 
                 generator_size=64, prior=PRIOR, clip_val=5.0, dropout=0.0, 
                 max_norm=200, deep_freeze=False, do_normalize_factors=True, 
                 factor_bias=False, device='cpu'):
        
        super(LFADS_MultiSession_Net, self).__init__(
            input_size=factor_size, 
            actor_size=factor_size, 
            prior=prior,
            g_encoder_size=g_encoder_size, 
            c_encoder_size=c_encoder_size,
            g_latent_size=g_latent_size, 
        ...
        for idx, (W_in, b_in, W_out, b_out) in enumerate(
            zip(W_in_list, b_in_list, W_out_list, b_out_list)
            ):

            for W, W_name in zip([W_in, W_out], ['in', 'out']):
                if W.shape[1] != self.factor_size:
                    raise ValueError(
                        f'Read {W_name} matrix should have dim 1 = '
                        f'{self.factor_size}, but has dims '
                        f'[{W.shape[0]}, {W.shape[1]}]'
                    )

            setattr(
                self, f'fc_input_{idx}', 
                nn.Linear(
                    in_features=W_in.shape[0], out_features=self.factor_size
                    )
                )
            setattr(
                self, f'fc_logrates_{idx}', 
                nn.Linear(
                    in_features=self.factor_size, out_features=W_in.shape[0]
                    )
                )
            
            getattr(self, f'fc_input_{idx}').weight.data = W_in.permute(1, 0)
            getattr(self, f'fc_input_{idx}').bias.data = b_in
            getattr(self, f'fc_logrates_{idx}').weight.data = W_out.permute(1, 0)
            getattr(self, f'fc_logrates_{idx}').bias.data = b_out
            
#             getattr(self, f'fc_input_{idx}').weight.requires_grad = False
#             getattr(self, f'fc_logrates_{idx}').weight.requires_grad = False
#             getattr(self, f'fc_input_{idx}').bias.requires_grad = False
#             getattr(self, f'fc_logrates_{idx}').bias.requires_grad = False
            

    def forward(self, input):
        aligned_input = getattr(
            self, f'fc_input_{input.session}'
            )(input).permute(1, 0, 2)

        factors, gen_inputs = super(LFADS_MultiSession_Net, self).forward(
            aligned_input
            )

        rates = getattr(
            self, f'fc_logrates_{input.session}'
            )(factors).exp().permute(1, 0, 2)
        recon = {
            'rates': rates,
            'data' : rates.clone(),
            }
...
class LFADS_Encoder(nn.Module):
    ...
    __init__(self, input_size, g_encoder_size, g_latent_size, c_encoder_size=0, 
    ...
    '''
    def __init__(self, input_size, g_encoder_size, g_latent_size, 
                 c_encoder_size=0, dropout=0.0, clip_val=5.0):
        ...
        # g Encoder BiRNN
        self.gru_g_encoder = nn.GRU(
            input_size=self.input_size, 
            hidden_size=self.g_encoder_size, 
            bidirectional=True
            )

        # g Linear mapping
        self.fc_g0_theta = nn.Linear(
            in_features=self.g_encoder_size * 2, 
            out_features=self.g_latent_size * 2
            )
        
        if self.c_encoder_size > 0:
            # c encoder BiRNN
            self.gru_c_encoder  = nn.GRU(
                input_size=self.input_size, 
                hidden_size=self.c_encoder_size, 
                bidirectional=True
                )
    ...
    def forward(self, input, hidden):
        ...
        out_gru_g_enc, hidden_gru_g_enc = self.gru_g_encoder(
            self.dropout(input), gru_g_encoder_init.contiguous()
            )
        hidden_gru_g_enc = self.dropout(
            hidden_gru_g_enc.clamp(min=-self.clip_val, max=self.clip_val)
            )
        hidden_gru_g_enc = torch.cat(
            (hidden_gru_g_enc[0], hidden_gru_g_enc[1]), dim=1
            )
        
        g0_mean, g0_logvar = torch.split(
            self.fc_g0_theta(hidden_gru_g_enc), self.g_latent_size, dim=1
            )
        
        if self.c_encoder_size > 0:
            out_gru_c_enc, hidden_gru_c_enc = self.gru_c_encoder(
                self.dropout(input), gru_c_encoder_init.contiguous()
                )
            out_gru_c_enc = out_gru_c_enc.clamp(
...
class LFADS_ControllerCell(nn.Module):
    ...
    def __init__(self, input_size, controller_size, u_latent_size, dropout=0.0, 
                 clip_val=5.0):
        ...
        self.gru_controller = rnn.LFADS_GenGRUCell(
            input_size=self.input_size, 
            hidden_size=self.controller_size
            )
        self.fc_u_theta = nn.Linear(
            in_features=self.controller_size, 
            out_features=self.u_latent_size * 2
            )
        
    def forward(self, input, hidden):
        controller_state = hidden
        controller_state = self.gru_controller(
            self.dropout(input), controller_state
            )
        controller_state = controller_state.clamp(-self.clip_val, self.clip_val)
        u_mean, u_logvar = torch.split(
            self.fc_u_theta(controller_state), self.u_latent_size, dim=1
            )
...
class LFADS_GeneratorCell(nn.Module):
    ...
    def __init__(self, input_size, generator_size, factor_size, dropout=0.0, 
                 clip_val=5.0, factor_bias=False):
        ...
        self.gru_generator = rnn.LFADS_GenGRUCell(
            input_size=input_size, hidden_size=generator_size
            )
        self.fc_factors = nn.Linear(
            in_features=generator_size, 
    ...
    def forward(self, input, hidden):
        ...
        generator_state = generator_state.clamp(
            min=-self.clip_val, max=self.clip_val
            )
