...
class VaLPACa_Net(nn.Module):
    ...
    def __init__(self, input_size,
        ...
        self.deep_model           = lfads.LFADS_Net(input_size      = self.input_size,
    ...
    def forward(self, input):
        ...
        for s in range(self.obs_pad):
            obs_u_mean, obs_u_logvar, obs_controller_state = self.obs_model.controller(torch.cat((out_obs_enc[s], obs_state), dim=1), obs_controller_state)
            ...
            obs_state, spike_state = self.obs_model.generator(torch.cat((obs_generator_state, torch.zeros_like(factor_state)), dim=1), obs_state)
...
class Calcium_Net(nn.Module):
    def __init__(self, input_size, encoder_size=128,
                 latent_size=64, controller_size=128, factor_size=4, pad=0,
...
class Calcium_Encoder(nn.Module):
    ...
    '''
    def __init__(self, input_size, encoder_size, pad=0, dropout= 0.0, clip_val= 5.0):
...
class AR1_Calcium(nn.Module):
    ...
    def __init__(self, parameters = {'gain': {'value' : 1.0, 'learnable': False},
        ...
        self.gain = value_to_tensor(parameters['gain']['value'], parameters['gain']['learnable'], device=device)
        self.bias = value_to_tensor(parameters['bias']['value'], parameters['bias']['learnable'], device=device)
